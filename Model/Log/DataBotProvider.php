...
class DataBotProvider extends AbstractDataProvider
{
    /** @var File */
    ...
    public function getData()
    {
        ...
        $moment = 'Today';
        $this->logFile = BP . '/var/botblacklist/log/' . LogBotService::LOG_BOT_FILENAME . '.log';
        $filters = $this->request->getParam('filters');
        if (isset($filters['logfile'])) {
            $this->logFile = BP . '/var/botblacklist/log/' . $filters['logfile'];
            $moment = 'Past';
        ...
        if ($this->fileDriver->isExists($this->logFile)) {
            ...
            $filterDetection = $filters['detection'] ?? null;
            ...
            foreach ($lines as $line) {
                ...
                [$date, $time, $ip, $userAgent, $detection] = explode('|', $line);
                ...
                // Filtro por detection
                if ($filterDetection && stripos($detection, trim($filterDetection, '%')) === false) {
                    continue;
                }
